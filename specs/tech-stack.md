...
- **File picking**: `file_picker ^11.0.3` — device file selection for the upload form (audio file + cover art image); no web-view fallback needed on the mobile targets.
...
- **file_picker ≥9 has no `FilePicker.platform`**: `file_picker` 9–11 replaced the instance-style `FilePicker.platform.pickFiles(...)` with a static `FilePicker.pickFiles(...)`; the class is `abstract final`, so consumers cannot subclass it. The test seam is `FilePickerPlatform.instance` (a settable `PlatformInterface` in `src/platform/file_picker_platform_interface.dart`, importable only via an internal path — `implementation_imports` lint suppressed in tests). Source: code inspection of `file_picker-11.0.3` on 2026-08-14.
