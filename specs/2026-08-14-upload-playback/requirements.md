...
- **Pinned versions this phase depends on** (executor verifies installed versions match in Group 6): `just_audio ^0.10.6` (resolved; plan wrote ^0.9.x), `audio_service ^0.18.19`, `just_audio_background ^0.0.1-beta.17`, `cached_network_image ^3.4.1`, `file_picker ^12.0.0` (Group 5 resolution — see Change Log; plan wrote ^8.x), `flutter_secure_storage ^11.0.0` (Group 5 resolution, was ^9.2.4), `@nestjs/platform-express ^11.x`, `Prisma ^6.x` (6.19.3 installed), `music-metadata ^7.14.0` (pinned — see above). Android `compileSdk = 37` (app build.gradle.kts).
...
- [2026-08-14] Group 5 (Android build): the first APK build failed — file_picker 11.0.3 skips KGP on AGP ≥ 9 but the Flutter template sets `android.builtInKotlin=false`, so its Kotlin never compiled (`cannot find symbol FilePickerPlugin`); flipping built-in Kotlin on breaks app_links 6.4.1 (applies KGP unconditionally). Fix: `file_picker ^12.0.0` + `flutter_secure_storage ^11.0.0` (win32 constraint) + `compileSdk = 37`. Full reasoning in tech-stack.md Change Log. `FilePicker.pickFile` (single-file API) used for audio + cover; tests build a `PlatformFile` subclass (abstract in 12.x) — full detail in tech-stack.md Known Issues.
- [2026-08-14] Group 5: on-device walkthrough PENDING — backend healthy (`/health` → `{"status":"ok","db":"up"}`), APK built with the walkthrough dart-defines, but phone `R3CR203BN8B` not connected and the stakeholder's real audio file not yet provided. Scriptable prep complete and committed; the 7 walkthrough steps await hardware.
