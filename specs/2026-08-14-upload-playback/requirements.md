...
- **Pinned versions this phase depends on** (executor verifies installed versions match in Group 6): `just_audio ^0.9.x`, `audio_service ^0.18.x`, `just_audio_background ^0.0.1-beta.x`, `cached_network_image ^3.x`, `file_picker ^8.x`, `@nestjs/platform-express ^11.x`, `Prisma ^6.x` (6.19.3 installed), `music-metadata ^7.14.0` (pinned — see above).
