...
## Target User
A single end user of the finished app: someone who signs up, uploads their own audio tracks (their own recordings, or audio they have rights to), and gets a full Spotify-grade experience (browse, search, play, organize) on top of that personal/shared catalog.

## Value Proposition
A pixel-close, production-grade clone of Spotify — Flutter client, NestJS backend, Postgres — where users authenticate, upload their own tracks with metadata, and browse/search/play/organize them with the real Spotify UI and interaction model.
...
4. **As a user**, I want to upload my own audio tracks with title/artist/album/cover art metadata, so that they become part of the playable catalog.
5. **As a listener**, I want full playback controls (play/pause/skip/seek/queue/shuffle/repeat) that persist in the background, so that listening feels seamless and matches the real Spotify experience.
6. **As a user**, I want to create, rename, delete, and reorder playlists, and like individual songs, so that I can organize my library the way I want.
